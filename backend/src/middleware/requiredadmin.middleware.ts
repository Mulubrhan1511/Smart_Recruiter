...
@Injectable()
export class RequiredAdminMiddleware implements NestMiddleware {
    ...
    async use(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            if(user.type !== 'admin') return res.status(401).json({error:"YOU must be Admin to access this route"})
            next();
        } catch (error) {
